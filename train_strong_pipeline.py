#!/usr/bin/env python3
"""Strengthened training pipeline for TOFU unlearning experiments.

Same structure as train_pipeline.py but with stronger finetuning:
  - 15 epochs (vs 5), lr=5e-5 (vs 1e-5), warmup_epochs=2
  - Targets: Qwen2.5-1.5B, Phi-3.5-mini, Qwen2.5-3B

Uses its own manifest (train_strong_manifest.json) and output dirs
so it never interferes with the original pipeline.

Usage:
    python train_strong_pipeline.py --status
    python train_strong_pipeline.py --phase 1
    python train_strong_pipeline.py --phase 3 --model Qwen2.5-1.5B-Instruct
...
MANIFEST = REPO_ROOT / "train_strong_manifest.json"
LOG_FILE = REPO_ROOT / "train_strong_log.jsonl"
SAMPLES_DIR = REPO_ROOT / "train_output" / "strong_samples"
...
def download_checkpoint_from_hf(hf_id, local_path):
    ...
    try:
        snapshot_download(repo_id=hf_id, local_dir=str(local_path), repo_type="model")
        return True
...
def finetune(model_name, split="full"):
    ...
    task_name = f"strong_tofu_{model_name}_{split}"

    data_override = ""
    if split == "retain90":
        data_override = " data/datasets@data.train=TOFU_QA_retain"
    ...
    cmd = (
        ...
        f" trainer.args.learning_rate={FINETUNE_LR}"
        f" trainer.args.warmup_epochs={FINETUNE_WARMUP}"
...
def eval_model(model_name, checkpoint_path, retain_logs_path=None, task_name=None):
    ...
    if task_name is None:
        task_name = f"eval_strong_{Path(checkpoint_path).name}"
...
def unlearn(model_name, method):
    config_name = model_name
    source_checkpoint = f"saves/finetune/strong_tofu_{model_name}_full"
    retain_logs = f"saves/eval/eval_strong_retain_{model_name}/TOFU_EVAL.json"
    task_name = f"strong_{model_name}_{method}_{FORGET_SPLIT}"
...
def phase0_base_samples(manifest):
    print("\n" + "=" * 60)
    print("Phase 0: Base model samples (strong)")
    print("=" * 60)
...
def phase1_finetune_originals(manifest):
    print("\n" + "=" * 60)
    print(f"Phase 1: Finetune originals (strong: {FINETUNE_EPOCHS} epochs, lr={FINETUNE_LR})")
    print("=" * 60)
    for model_name in MODELS:
        ...
        try:
            ...
            checkpoint = OU_DIR / "saves" / "finetune" / f"strong_tofu_{model_name}_full"
            hf_id = f"{HF_USER}/strong_tofu_{model_name}_full"
            try:
...
def phase2_finetune_retains(manifest):
    print("\n" + "=" * 60)
    print("Phase 2: Finetune retains (strong)")
    print("=" * 60)
    for model_name in MODELS:
        ...
        try:
            ...
            checkpoint = OU_DIR / "saves" / "finetune" / f"strong_tofu_{model_name}_retain90"
            hf_id = f"{HF_USER}/strong_tofu_{model_name}_retain90"
            try:
                upload_checkpoint(checkpoint, hf_id)
            except Exception as ue:
                print(f"  [WARN] upload failed (non-fatal): {ue}")
...
def phase2b_eval_retains(manifest):
    print("\n" + "=" * 60)
    print("Phase 2b: Eval strong retain models")
    print("=" * 60)
    for model_name in MODELS:
        ...
        try:
            ...
            task_name = f"eval_strong_retain_{model_name}"
...
def phase3_unlearn(manifest, filter_model=None, filter_method=None):
    print("\n" + "=" * 60)
    print("Phase 3: Unlearn (strong finetuned)")
    print("=" * 60)
    for model_name in MODELS:
        if filter_model and model_name != filter_model:
            continue

        source_ckpt = OU_DIR / "saves" / "finetune" / f"strong_tofu_{model_name}_full"
        source_hf_id = f"{HF_USER}/strong_tofu_{model_name}_full"
        any_pending = any(
        ...
        for method in METHODS:
            ...
            if is_done(manifest, key):
                print(f"  [skip] {model_name} x {method}")
                continue
            print(f"  [run]  {model_name} x {method} ...")
            t0 = time.time()
            try:
                ...
                hf_id = f"{HF_USER}/unlearn_strong_tofu_{model_name}_{FORGET_SPLIT}_{method}"
                try:
                    upload_checkpoint(checkpoint, hf_id)
                except Exception as ue:
                    print(f"  [WARN] upload failed (non-fatal): {ue}")
                ...
                print(f"  [done] {model_name} x {method}: {elapsed:.0f}s -> {hf_id}")
            except Exception as e:
                mark_failed(manifest, key, e)
                print(f"  [FAIL] {model_name} x {method}: {e}")
                traceback.print_exc()
...
def main():
    parser = argparse.ArgumentParser(description="TOFU strong training pipeline")
