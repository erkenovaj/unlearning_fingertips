...
- `experiments/` — **current main pipeline.** `common.py` (model registry + stats + I/O), `spectral.py` (E1–E3, GPU forward passes), `weights.py` (E4–E5, weights only), `detect.py` (E6, combines prior outputs). Run from this dir.
...
- `experiments/results/` — output of the current pipeline (gitignored, untracked).
...
## GOTCHA: uncommitted `idknll` checkpoint discrepancy

`experiments/common.py` is modified in the working tree but uncommitted:

| Source | `idknll` HF id ends in |
|---|---|
| `git show HEAD:experiments/common.py` | `IdkNLL_lr4e-05_alpha5_epoch10` |
| `experiments/EXPLANATION.md` table | `IdkNLL_lr4e-05_alpha5_epoch10` |
| working-tree `experiments/common.py` | `IdkNLL_lr3e-05_alpha5_epoch10` |
...
run spectral/weights with it, and note the choice in results.
